...
export default function ContinuousCarousel() {
    ...
    // Track the number of active video playbacks to pause/resume the marquee safely
    const [playingCount, setPlayingCount] = useState<number>(0);
    // Safe React state tracking for failed video renders
    ...
    const infinitePosts = posts.length > 0 ? [...posts, ...posts] : [];
            ...
            {!loading && !error && posts.length > 0 && (
                ...
                >
                    {infinitePosts.map((post: any, idx: number) => {
                        const uniqueKey = `${post._id || post.id || idx}-${idx}`;
                        ...
                        return (
                            <div
                                key={uniqueKey}
                                className="w-[380px] flex-shrink-0 group cursor-pointer  border border-border/40 rounded-[2rem] overflow-hidden shadow-xl flex flex-col transition-all duration-300 hover:border-primary/30"
                            >
